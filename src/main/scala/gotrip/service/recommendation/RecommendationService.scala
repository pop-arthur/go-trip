...
object RecommendationService {
  ...
  ): RecommendationService[F] =
    new RecommendationService[F] {
      import RecommendationServiceError.*

      override def forTrip(userId: UserId, tripId: TripId): F[Either[RecommendationServiceError, List[Recommendation]]] =
        (for {
      ...
      override def forOrder(userId: UserId, orderId: OrderId): F[Either[RecommendationServiceError, List[Recommendation]]] =
        (for {
      ...
      private def rankTripServices(
        ...
        }.sortBy(rec => (-rec.score.getOrElse(0.0), rec.service.title.value))
      }
