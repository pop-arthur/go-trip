  ...
  def make[F[_]: Sync: Clock: GeneratedData](
    ...
    new TripService[F] {
      ...
      override def listByUser(userId: UserId, params: TripSearchParams): F[List[Trip]] =
        repo.listByUser(userId, params)

      override def findByUser(userId: UserId, tripId: TripId): F[Either[TripServiceError, Trip]] =
        EitherT.fromOptionF(repo.findByUser(userId, tripId), TripNotFound(tripId)).value

      override def create(userId: UserId, trip: TripCreate): F[Either[TripServiceError, Trip]] =
        (for {
          _ <- validateDateRange(trip.start_date.value, trip.end_date.value)
          created <- EitherT.liftF(repo.create(userId, trip))
          _ <- EitherT.liftF(achievementEngine.checkAndUnlock(userId, AchievementEvent.TripCreated(created)))
        } yield created).value

      override def update(userId: UserId, tripId: TripId, trip: TripUpdate): F[Either[TripServiceError, Trip]] =
        (for {
          current <- EitherT.fromOptionF(repo.findByUser(userId, tripId), TripNotFound(tripId))
          _ <- validateDateRange(nextStartDate(current, trip), nextEndDate(current, trip))
          updated <- EitherT.fromOptionF(repo.update(userId, tripId, trip), TripNotFound(tripId))
      ...
      override def delete(userId: UserId, tripId: TripId): F[Either[TripServiceError, Unit]] =
        (for {
          deleted <- EitherT.liftF(repo.delete(userId, tripId))
