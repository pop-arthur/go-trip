...
trait OrderFileService[F[_]] {
  def listByOrder(userId: UserId, orderId: OrderId): F[Either[OrderFileServiceError, List[OrderFile]]]
  def findByOrder(userId: UserId, orderId: OrderId, fileId: OrderFileId): F[Either[OrderFileServiceError, OrderFile]]
  def create(userId: UserId, orderId: OrderId, file: OrderFileCreate): F[Either[OrderFileServiceError, OrderFile]]
  def delete(userId: UserId, orderId: OrderId, fileId: OrderFileId): F[Either[OrderFileServiceError, Unit]]
}

object OrderFileService {
  def make[F[_]: Sync: Clock: GeneratedData](repo: OrderFileRepository[F]): OrderFileService[F] =
    new OrderFileService[F] {
      import OrderFileServiceError._

      private val uploadDir = "uploads"

      override def listByOrder(userId: UserId, orderId: OrderId): F[Either[OrderFileServiceError, List[OrderFile]]] =
        (for {
          _ <- ensureOrderExists(userId, orderId)
          files <- EitherT.liftF(repo.listByOrder(userId, orderId))
        } yield files).value
      ...
      override def findByOrder(
        ...
        EitherT.fromOptionF(repo.findByOrder(userId, orderId, fileId), OrderFileNotFound(fileId)).value
      ...
      override def create(
        ...
        (for {
          ...
          created <- EitherT.fromOptionF(repo.create(userId, materialized), OrderNotFound(orderId))
        } yield created).value
      ...
      override def delete(
        ...
        (for {
          _ <- ensureOrderExists(userId, orderId)
          fileOpt <- EitherT.liftF(repo.findByOrder(userId, orderId, fileId))
          file <- EitherT.fromOption(fileOpt, OrderFileNotFound(fileId))
          _ <- EitherT.liftF(deleteLocalFile(file.file_url.value))
          deleted <- EitherT.liftF(repo.delete(userId, orderId, fileId))
      ...
      private def ensureOrderExists(userId: UserId, orderId: OrderId): EitherT[F, OrderFileServiceError, Unit] =
        EitherT {
          repo.orderExistsForUser(userId, orderId).map { exists =>
