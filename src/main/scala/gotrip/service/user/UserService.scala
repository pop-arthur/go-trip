...
final class UserService[F[_]: Sync: Clock](
  ...
  def update(user: User): F[Int] =
    GeneratedData.now[F].flatMap(now => repo.update(user.copy(updatedAt = now)))
  def delete(id: UserId): F[Int] = repo.delete(id)

  def addRole(userId: UserId, role: Role): F[Int] =
    GeneratedData.now[F].flatMap(now => repo.addRole(userId, role, now, now))
