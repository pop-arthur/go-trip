...
trait OrderService[F[_]] {
  def listByTrip(userId: UserId, tripId: TripId, params: OrderSearchParams): F[Either[OrderServiceError, List[Order]]]
  def findByUser(userId: UserId, orderId: OrderId): F[Either[OrderServiceError, Order]]
  def create(userId: UserId, tripId: TripId, order: OrderCreate): F[Either[OrderServiceError, Order]]
  def update(userId: UserId, orderId: OrderId, order: OrderUpdate): F[Either[OrderServiceError, Order]]
  def delete(userId: UserId, orderId: OrderId): F[Either[OrderServiceError, Unit]]
  def updateStatus(userId: UserId, orderId: OrderId, update: OrderStatusUpdate, source: OrderStatusEventSource): F[Either[OrderServiceError, Order]]
}
...
object OrderService {
  def make[F[_]: Sync: Clock: GeneratedData](
    ...
    new OrderService[F] {
      ...
      override def findByUser(userId: UserId, orderId: OrderId): F[Either[OrderServiceError, Order]] =
      ...
      override def delete(userId: UserId, orderId: OrderId): F[Either[OrderServiceError, Unit]] =
        (for {
